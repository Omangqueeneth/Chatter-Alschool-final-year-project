...
const PostsCard = ({ post}) => {
  if (!post) {
    return null; // Or return some fallback UI if post is undefined
  }
  const { title, desc, created, postImg, id: postId, userId, username } = post;
  const { currentUser } = Blog(); // Assuming this returns the current user
    ...
    <section>
      ...
      <div className="flex items-center justify-between w-full md:w-[70%] mt-[2rem] md:mt-0">
        <p className="text-xs text-gray-600">
          {readTime(desc)} min read . {moment(created).format("MMM DD")}
        </p>
